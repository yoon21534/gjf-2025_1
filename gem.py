...
def get_all_watch_records():
    ...
    if not df.empty:
        ...
        # TMDB 웹 URL 컬럼 추가 (사용하지 않으므로 주석 처리하거나 제거 가능)
...
<style>
    ...
    .box-office-rank {
        ...
        border-radius: 8px;
        border-left: 5px solid #007bff; /* 파란색 왼쪽 테두리 */
        margin-bottom: 1rem;
        font-size: 1.1em;
        font-weight: bold;
        color: #333333;
    ...
    /* Link Button (Streamlit default) */
    a.st-emotion-cache-19p006b { /* This class name might change with Streamlit versions */
        color: #007bff;
        text-decoration: none;
        font-weight: bold;
    }
    a.st-emotion-cache-19p006b:hover {
        text-decoration: underline;
    }
</style>
""", unsafe_allow_html=True)

# Define menu options and their default index (이미지 기반으로 수정)
MENU_OPTIONS = ["🏠 메인", "📚 나의 영화 목록", "📊 통계 & 분석", "📅 월말 결산", "🎊 연말 결산"]
DEFAULT_MENU_INDEX = 0 # "🏠 메인"이 첫 번째 항목 (인덱스 0)

# --- 사이드바 메뉴 ---
st.sidebar.title("메뉴") # 이미지에 맞춰 제목 변경
menu_selection = st.sidebar.radio(
    "", # 라디오 버튼 자체의 라벨은 비워둡니다.
    MENU_OPTIONS,
    index=DEFAULT_MENU_INDEX 
)

# --- 메인 페이지 (박스오피스 + 영화 기록) ---
if menu_selection == "🏠 메인": # 메뉴 항목 이름 변경에 따라 조건문 수정
...
# --- 2. 나의 영화 목록 페이지 ---
elif menu_selection == "📚 나의 영화 목록": # 메뉴 항목 이름 변경에 따라 조건문 수정
...
# --- 3. 월말 결산 페이지 ---
elif menu_selection == "📅 월말 결산": # 메뉴 항목 이름 변경에 따라 조건문 수정
    st.title("📅 월말 결산")
...
# --- 4. 연말 결산 페이지 ---
elif menu_selection == "🎊 연말 결산": # 메뉴 항목 이름 변경에 따라 조건문 수정
    st.title("🎊 연말 결산")
...
elif menu_selection == "📊 통계 & 분석": # 메뉴 항목 이름 변경에 따라 조건문 수정
