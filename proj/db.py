            ...
            for i, movie in enumerate(search_results[:10]):
                ...
                if movie_details:
                    with st.form("watch_record_form"):
                        ...
                        # 이전 장소들을 버튼으로 표시 (선택사항)
                        if all_previous_locations:
                            st.write("💡 이전에 입력한 장소:")
                            cols = st.columns(min(len(all_previous_locations), 4))
                            for i, location in enumerate(all_previous_locations[:4]):
                                with cols[i % 4]:
                                    if st.form_submit_button(f"📍 {location}", key=f"location_{i}"):
                                        watch_method_detail = location
                        ...
                        if movie_details:
                            with st.form(f"box_office_record_{movie['rank']}"):
                                ...
                                if all_previous_locations:
                                    ...
                                    for i, location in enumerate(all_previous_locations[:4]):
                                        with cols[i % 4]:
                                            if st.form_submit_button(f"📍 {location}", key=f"box_location_{movie['rank']}_{i}"):
                                                watch_method_detail = location
                    ...
                    with col2:
                        ...
                        with col_link:
                            if row['tmdb_web_url'] and row['tmdb_web_url'] != '#':
                                st.link_button("TMDB 보기", row['tmdb_web_url'])
...
# 연말 결산
    ...
    if not yearly_movies.empty:
        ...
        if not yearly_genre_counts.empty:
            st.bar_chart(yearly_genre_counts)
                ...
                for i, movie in enumerate(recommendations, 1):
                    with st.container():
                        ...
                        with col2:
                            ...
                            col_btn1, col_btn2 = st.columns(2)
                            with col_btn1:
                                if st.button(f"관심 있어요! 📝", key=f"interested_{movie['id']}"):
                                    # 관심 영화를 바로 기록 페이지로 이동할 수 있게
                                    st.session_state.selected_recommendation = movie
                                    st.session_state.current_page = "영화 기록하기"
                                    st.rerun()
